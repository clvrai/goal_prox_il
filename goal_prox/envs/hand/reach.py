...
class HandReachEnv(hand_env.HandEnv, utils.EzPickle):
    ...
    def _render_callback(self):
        ...
        for finger_idx in range(5):
            geom_name = 'target{}'.format(finger_idx)
            geom_id = self.sim.model.geom_name2id(geom_name)
            self.sim.model.geom_pos[geom_id][0:3] = goal[finger_idx][:]

        # Visualize finger positions.
        achieved_goal = self._get_achieved_goal().reshape(5, 3)
        for finger_idx in range(5):
            geom_name = 'finger{}'.format(finger_idx)
            geom_id = self.sim.model.geom_name2id(geom_name)
            self.sim.model.geom_pos[geom_id][0:3] = achieved_goal[finger_idx][:]
