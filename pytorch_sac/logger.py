...
class Logger(object):
    ...
    def _should_log(self, key, step, log_frequency):
        # To handle uneven logging intervals, for instance on episode
        # completion.
        ret = True
        log_frequency = log_frequency or self._log_frequency
        if key in self.last_log_step:
            step_diff = step - self.last_log_step[key]
            ret = step_diff > log_frequency
        if ret:
            self.last_log_step[key] = step
        return ret
        #log_frequency = log_frequency or self._log_frequency
    ...
    def log(self, key, value, step, n=1, log_frequency=None):
        if not self._should_log(key, step, log_frequency):
            return
